...
def generate_binary_matrix(m, n, P):
    """ 生成 m×n 的行稀疏随机矩阵（元素为0或1）,每行非零元素个数固定 """
...
def simulation_for_single_k_p_LOD(k, n, p, LOD_list, simu_num):
    ...
    for simu in tqdm(range(1, simu_num + 1)):
        ...
        P = int(n * p)
        A_full = generate_binary_matrix(n, n, P)
        # 对 A_full 的每一行归一化，避免除零
        A_full_normalized = A_full / P
...
if __name__ == "__main__":
    ...
    np.save("/remote-home/share/iot_lijianing/Mix_Detection/results/combinatorial_iv_fix/combinatorial_iv_fix_data_n{}/n{}_kratio{}.npy".format(n, n, args.k_ratio), simulation_results)
    print("success")
